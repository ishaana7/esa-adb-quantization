...
class ErrorWindow:
    ...
    def compare_to_epsilon(self, errors_all, inverse=False):
        ...
        if not (self.sd_e_s > (.02 * self.sd_values) or max(self.e_s)
                > (.02 * self.inter_range)) or not max(self.e_s) > 0.02:
            return
    ...
    def prune_anoms(self, inverse=False):
        ...
        if len(i_to_remove) > 0:
            E_sE_seq = np.delete(E_seq, [int(i) for i in i_to_remove], axis=0)
